...
/* -------------------------------------------------------------------------- *
 * WebGPU Example - Texture Cubemap
 *
 * Loads a cubemap from a Horizontal Cross PNG image and displays it as a
 * skybox (background) and as a reflection on a selectable 3D object.
 * Face extraction from the cross image is done entirely on the GPU via a
 * fragment shader render pass (one per face), avoiding all CPU copies.
 ...
 *
 * Ported from Sascha Willems' Vulkan example "texturecubemap"
...
/* Main cubemap rendering shader (skybox + reflection) */
static const char* texture_cubemap_shader_wgsl;
...
/* Horizontal cross source image: 4 faces wide × 3 faces tall */
#define CUBEMAP_FACE_SIZE (512)
/* Buffer for the raw compressed PNG file (the 2.5 MB cross PNG) */
#define CROSS_FILE_BUF_SIZE (4u * 1024u * 1024u)
...
typedef struct {
  ...
  mat4 inverse_model_view; /* 64 bytes */
...
static struct {
  ...
  /* Raw PNG file buffer for sokol_fetch (cross image) */
#ifndef __WAJIC__
  /* Native only: pre-allocated fetch buffer (WAjic uses dynamic allocation) */
  uint8_t cross_file_buf[CROSS_FILE_BUF_SIZE];
#endif
  /* Decoded cross image pixels (stb_image allocated, freed after GPU upload) */
  uint8_t* cross_pixels;
  int cross_width;
  int cross_height;
  bool cross_loaded;
...
#ifdef __WAJIC__
  int wajic_models_loaded; /* counts loaded models (skybox + objects) */
  ...
  .depth_stencil_attachment = {
    ...
    /* No stencil aspect for Depth24Plus — leave stencil ops undefined */
...
/**
 * Extract all 6 cubemap faces from a horizontal cross image on the GPU.
 *
 * The cross layout (4 faces wide × 3 faces tall) is:
 *   col:  0    1    2    3
 *   row 0: .   +Y   .    .
 *   row 1: -X  +Z  +X   -Z
 *   row 2: .   -Y   .    .
 *
 * The face index mapping used by WebGPU:
 *   0 = +X (col 2, row 1)   1 = -X (col 0, row 1)
 *   2 = +Y (col 1, row 0)   3 = -Y (col 1, row 2)
 *   4 = +Z (col 1, row 1)   5 = -Z (col 3, row 1)
 *
 * Algorithm:
 *  1. Upload the full cross image as a temporary 2D source texture.
 *  2. Build a fragment-shader pipeline that maps face UV [0,1]² to the
 *     corresponding UV region in the cross (no CPU copies).
 *  3. Run one render pass per face, targeting that layer of the cubemap.
 *  4. Generate the full mip chain via wgpu_generate_mipmaps.
 *  5. Release all temporary resources.
 */
static void extract_cubemap_faces_from_cross(wgpu_context_t* wgpu_context)
{
  const uint32_t cross_w   = (uint32_t)state.cross_width;
  const uint32_t cross_h   = (uint32_t)state.cross_height;
  const uint32_t face_size = cross_w / 4u; /* = cross_h / 3 */

  /* ---------------------------------------------------------------------- *
   * 1. Create and upload the source 2D cross texture
   * ---------------------------------------------------------------------- */
  ...
  wgpuQueueWriteTexture(wgpu_context->queue,
                        ...
                        state.cross_pixels, (size_t)(cross_w * cross_h * 4u),
                        &(WGPUTexelCopyBufferLayout){
                          .offset       = 0,
                          .bytesPerRow  = cross_w * 4u,
                          .rowsPerImage = cross_h,
                        },
                        &(WGPUExtent3D){.width              = cross_w,
                                        .height             = cross_h,
                                        .depthOrArrayLayers = 1});

  /* Free CPU pixels now — they are on the GPU */
  image_free(state.cross_pixels);
  state.cross_pixels = NULL;
  ...
  /* ---------------------------------------------------------------------- *
   * 2. Create the destination cubemap texture (mip 0 filled by extraction,
   *    remaining mips generated afterwards)
   * ---------------------------------------------------------------------- */
  ...
  state.cubemap.handle = wgpuDeviceCreateTexture(
    ...
    &(WGPUTextureDescriptor){
      ...
      .dimension     = WGPUTextureDimension_2D,
      .size          = {face_size, face_size, NUM_CUBEMAP_FACES},
      .format        = WGPUTextureFormat_RGBA8Unorm,
  ...
  /* ---------------------------------------------------------------------- *
   * 3. Build the face extraction render pipeline
   * ---------------------------------------------------------------------- */
  ...
  /* Single bind group — same source texture for all 6 faces */
  WGPUBindGroup extract_bg = wgpuDeviceCreateBindGroup(
    wgpu_context->device,
    &(WGPUBindGroupDescriptor){
      .label      = STRVIEW("Extract - Bind group"),
      .layout     = extract_bgl,
      .entryCount = 2,
      .entries    = (WGPUBindGroupEntry[]){
        {.binding = 0, .sampler = src_sampler},
  ...
  for (uint32_t face = 0; face < NUM_CUBEMAP_FACES; ++face) {
    ...
    /* instance_index = face (via firstInstance argument) */
  ...
  /* ---------------------------------------------------------------------- *
   * 6. Create the permanent cube texture view and sampler
   * ---------------------------------------------------------------------- */
  ...
  /* ---------------------------------------------------------------------- *
   * 7. Release all temporary extraction resources
   * ---------------------------------------------------------------------- */
  ...
  WGPU_RELEASE_RESOURCE(Sampler, src_sampler)
  WGPU_RELEASE_RESOURCE(TextureView, src_view)
  wgpuTextureDestroy(src_tex);
  WGPU_RELEASE_RESOURCE(Texture, src_tex)
}

/* -------------------------------------------------------------------------- *
 * Asynchronous horizontal-cross cubemap loading (single PNG fetch)
 * -------------------------------------------------------------------------- */
...
static void cross_fetch_callback(const sfetch_response_t* response)
{
  if (!response->fetched || !response->data.ptr || !response->data.size) {
    printf("[texture_cubemap] Failed to fetch cross image, error: %d\n",
           response->error_code);
    return;
  }
  ...
  if (!state.cross_pixels) {
    printf("[texture_cubemap] Failed to decode horizontal cross PNG\n");
    return;
  }

  state.cross_width  = width;
  state.cross_height = height;
  state.cross_loaded = true;

  printf("[texture_cubemap] Cross image decoded: %dx%d (%d channels)\n", width,
         height, channels);
}
...
{
  if (!response->fetched) {
    printf("[texture_cubemap] model fetch failed, error: %d\n",
           response->error_code);
    return;
  }
...
static void load_models(wgpu_context_t* wgpu_context)
{
...
#ifdef __WAJIC__
  /* WAjic: models are loaded asynchronously via sfetch with NULL buffers.
   * GPU buffers are created in frame() once all models have loaded. */
  ...
  /* Index 0 = skybox (cube.gltf), indices 1..NUM_OBJECTS = selectable objects
   */
...
static void init_pipelines(wgpu_context_t* wgpu_context)
{
  ...
  WGPUShaderModule shader = wgpu_create_shader_module(
    wgpu_context->device, texture_cubemap_shader_wgsl);

  /* Vertex attributes: position (skybox + reflect) and normal (reflect only).
   * Both pipelines use the same full gltf_vertex_t stride so the buffer
   * layout is identical; the reflect VS reads normal via location(1). */
  ...
  state.reflect_pipeline = wgpuDeviceCreateRenderPipeline(
    wgpu_context->device,
    &(WGPURenderPipelineDescriptor){
      .label  = STRVIEW("Reflect - Render pipeline"),
      .layout = state.pipeline_layout,
      .vertex = (WGPUVertexState){
        .module      = shader,
...
static void render_gui(wgpu_context_t* wgpu_context)
{
  ...
  igBegin("Cube Map Textures", NULL, ImGuiWindowFlags_AlwaysAutoResize);
...
static int init(wgpu_context_t* wgpu_context)
{
  ...
  /* Sokol fetch: 1 cross PNG + 5 models (WAjic also loads models via sfetch) */
  ...
  /* Start async fetch for the horizontal cross PNG */
  fetch_cubemap_cross();
...
static int frame(wgpu_context_t* wgpu_context)
{
  ...
  /* When the cross PNG is decoded, extract faces and build the cubemap */
  if (!state.cubemap.is_ready && state.cross_loaded) {
    extract_cubemap_faces_from_cross(wgpu_context);
    state.cross_loaded     = false;
...
static const char* face_extract_shader_wgsl = CODE(
   ...
   *
   * The vertex shader emits a fullscreen triangle using the standard trick:
   *   pos[0] = (-1,-1),  pos[1] = (-1, 3),  pos[2] = ( 3,-1)
   * UV is derived as: xy * vec2f(0.5, -0.5) + 0.5
   * This maps NDC top-left (-1,+1) -> UV (0,0) (top-left of image). The Y
   * component is negated because WebGPU NDC has Y up while texture V is down.
   *
   * The face index is passed via @builtin(instance_index) (firstInstance
   * argument of Draw). This lets a single bind group serve all 6 passes.
   */
  ...
  fn vs_extract(
    ...
    // Map NDC -> texture UV with Y flip (top of screen = V=0)
  ...
  fn fs_extract(in : VSOutput) -> @location(0) vec4f {
    // Map face-local UV [0,1]^2 to the correct region in the cross image
...
// clang-format off
static const char* texture_cubemap_shader_wgsl = CODE(
  /* -------------- Shared UBO -------------------------------------------- */
  struct UBO {
    projection      : mat4x4f,
    model_view      : mat4x4f,
  ...
  fn fs_reflect(in : ReflectVertexOutput) -> @location(0) vec4f {
    ...
    // Sample the cubemap with an optional LOD bias.
    ...
    let color = textureSampleBias(cubeTexture, cubeSampler, cR_sample, ubo.lod_bias);
