                                  ...
                                  const uint16_t* heightdata, int hm_dim)
{
  ...
  state.terrain.wire_index_buffer = wgpuDeviceCreateBuffer(
    device, &(WGPUBufferDescriptor){
              .label = STRVIEW("Terrain Wire - Index buffer"),
              .usage = WGPUBufferUsage_Index | WGPUBufferUsage_CopyDst,
              .size  = wib_size,
              .mappedAtCreation = true,
            });
  ...
  state.terrain.coarse_index_buffer = wgpuDeviceCreateBuffer(
    device, &(WGPUBufferDescriptor){
              .label = STRVIEW("Terrain Coarse - Index buffer"),
              .usage = WGPUBufferUsage_Index | WGPUBufferUsage_CopyDst,
              .size  = ci_size,
              .mappedAtCreation = true,
            });
...
static void create_bind_group_layouts(wgpu_context_t* wgpu_context)
{
  ...
  state.bg_layouts.terrain = wgpuDeviceCreateBindGroupLayout(
    device, &(WGPUBindGroupLayoutDescriptor){
              .label      = STRVIEW("Terrain - Bind group layout"),
              .entryCount = 5,
