...
create_seaweed_pipeline(WGPUDevice device, diffuse_pipeline_desc_t* desc)
{
  /* Shader module */
  const WGPUShaderModule shader_module
...
#define NUM_FACES (6)
  ...
  for (uint8_t face = 0; face < NUM_FACES; ++face) {
    ...
    bitmaps->depth      = 4;
    bitmaps->mip_levels = 1;
    stbi_set_flip_vertically_on_load(true);
    bitmap->pixels = stbi_load(urls[face], &bitmaps->width, &bitmaps->height,
                               &bitmaps->channels, bitmaps->depth);
  }
