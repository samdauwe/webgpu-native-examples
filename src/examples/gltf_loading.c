...
void gltf_model_load_images(gltf_model_t* model, cgltf_data* data)
{
  ...
  for (uint32_t i = 0; i < model->image_count; ++i) {
    ...
    img->texture = wgpu_texture_load_with_stb(wgpu_context, image_uri);
  }
}
...
static void prepare_pipelines(wgpu_context_t* wgpu_context)
{
  ...
  // Vertex input binding and attributes (=> Input assembly)
  WGPU_VERTSTATE(gltf_loading, sizeof(gltf_vertex_t),
  ...
  solid_pipeline = wgpuDeviceCreateRenderPipeline(
    ...
    &(WGPURenderPipelineDescriptor){
      ...
      .vertexState            = &vert_state_gltf_loading,
      .sampleCount            = 1,
