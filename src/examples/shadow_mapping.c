...
typedef struct stanford_dragon_mesh_t {
  struct {
    ...
    size_t count; // number of vertices (should be 5205)
  } vertices;
  struct {
    uint16_t data[CELL_COUNT_RES_4][3];
    size_t count; // number of faces (should be 11102)
  } triangles;    // triangles
...
static int
stanford_dragon_mesh_init(stanford_dragon_mesh_t* stanford_dragon_mesh)
{
  ASSERT(stanford_dragon_mesh)
...
/**
 * @brief This example shows how to sample from a depth texture to render
 * shadows.
 * @see
 * https://github.com/austinEng/webgpu-samples/blob/main/src/pages/samples/shadowMapping.ts
 * @see stanford-dragon: https://github.com/hughsk/stanford-dragon
 */
