...
static int GenerateInitialVerticesIndexList(STriInfo pTriInfos[], int piTriList_out[],
...
static void InitTriInfo(STriInfo pTriInfos[], const int piTriListIn[],
                        const SMikkTSpaceContext *pContext, const int iNrTrianglesIn);
static int Build4RuleGroups(STriInfo pTriInfos[], SGroup pGroups[], int piGroupTrianglesBuffer[],
...
static tbool GenerateTSpaces(STSpace psTspace[], const STriInfo pTriInfos[], const SGroup pGroups[],
                             const int iNrActiveGroups, const int piTriListIn[],
                             const float fThresCos, const SMikkTSpaceContext *pContext);
...
// degen triangles
static void DegenPrologue(STriInfo pTriInfos[], int piTriList_out[], const int iNrTrianglesIn,
                          const int iTotTris);
static void DegenEpilogue(STSpace psTspace[], STriInfo pTriInfos[], int piTriListIn[],
                          const SMikkTSpaceContext *pContext, const int iNrTrianglesIn,
                          const int iTotTris);
    ...
    for (f = 0; f < iNrFaces; f++) {
        ...
        for (i = 0; i < verts; i++) {
            ...
            if (pContext->m_pInterface->m_setTSpace != NULL) {
                pContext->m_pInterface->m_setTSpace(pContext, tang, bitang, pTSpace->fMagS,
                                                    pTSpace->fMagT, pTSpace->bOrient, f, i);
            }
...
static void MergeVertsFast(int piTriList_in_and_out[], STmpVert pTmpVert[],
                           const SMikkTSpaceContext *pContext, const int iL_in, const int iR_in);
static void MergeVertsSlow(int piTriList_in_and_out[], const SMikkTSpaceContext *pContext,
    ...
    if (fSep >= fvMax[channel] || fSep <= fvMin[channel]) {
        ...
        for (l = iL_in; l <= iR_in; l++) {
            ...
            while (l2 < l && bNotFound) {
                ...
                if (vP.x == vP2.x && vP.y == vP2.y && vP.z == vP2.z && vN.x == vN2.x &&
                    vN.y == vN2.y && vN.z == vN2.z && vT.x == vT2.x && vT.y == vT2.y &&
                    vT.z == vT2.z) {
                    bNotFound = TFALSE;
                } else {
                    ++l2;
...
static void BuildNeighborsFast(STriInfo pTriInfos[], SEdge *pEdges, const int piTriListIn[],
                               const int iNrTrianglesIn);
...
static void InitTriInfo(STriInfo pTriInfos[], const int piTriListIn[],
                        const SMikkTSpaceContext *pContext, const int iNrTrianglesIn) {
    int f = 0, i = 0, t = 0;
    // pTriInfos[f].iFlag is cleared in GenerateInitialVerticesIndexList() which is called before
    // this function.
        ...
        if (iFO_a == iFO_b) // this is a quad
        {
            ...
            if ((bIsDeg_a || bIsDeg_b) == TFALSE) {
                const tbool bOrientA =
                    (pTriInfos[t].iFlag & ORIENT_PRESERVING) != 0 ? TTRUE : TFALSE;
                const tbool bOrientB =
                    (pTriInfos[t + 1].iFlag & ORIENT_PRESERVING) != 0 ? TTRUE : TFALSE;
                ...
                if (bOrientA != bOrientB) {
                    ...
                    // force match
                    {
                        ...
                        pTriInfos[t1].iFlag |=
                            (pTriInfos[t0].iFlag & ORIENT_PRESERVING); // copy bit
                    }
                }
            }
            t += 2;
...
static tbool AssignRecur(const int piTriListIn[], STriInfo psTriInfos[], const int iMyTriIndex,
                         SGroup *pGroup);
    ...
    for (f = 0; f < iNrTrianglesIn; f++) {
        for (i = 0; i < 3; i++) {
            ...
            if ((pTriInfos[f].iFlag & GROUP_WITH_ANY) == 0 &&
                ...
                {
                    const tbool bAnswer = AssignRecur(piTriListIn, pTriInfos, neigh_indexL,
                ...
                {
                    const tbool bAnswer = AssignRecur(piTriListIn, pTriInfos, neigh_indexR,
                                                      pTriInfos[f].AssignedGroup[i]);
...
static STSpace EvalTspace(int face_indices[], const int iFaces, const int piTriListIn[],
                          const STriInfo pTriInfos[], const SMikkTSpaceContext *pContext,
    ...
    for (g = 0; g < iNrActiveGroups; g++) {
        ...
        for (i = 0; i < pGroup->iNrFaces; i++) // triangles
        {
            ...
            for (j = 0; j < pGroup->iNrFaces; j++) {
                ...
                {
                    const tbool bAny =
                        ((pTriInfos[f].iFlag | pTriInfos[t].iFlag) & GROUP_WITH_ANY) != 0 ? TTRUE
                                                                                          : TFALSE;
...
static void QuickSortEdges(SEdge *pSortBuffer, int iLeft, int iRight, const int channel,
                           unsigned int uSeed);
static void GetEdge(int *i0_out, int *i1_out, int *edgenum_out, const int indices[],
                    const int i0_in, const int i1_in);
    ...
    while (t < iNrTrianglesIn && bStillFindingGoodOnes) {
        ...
        if (bIsGood) {
            ...
            while (bJustADegenerate && iNextGoodTriangleSearchIndex < iTotTris) {
                const tbool bIsGood =
                    (pTriInfos[iNextGoodTriangleSearchIndex].iFlag & MARK_DEGENERATE) == 0 ? TTRUE
                                                                                           : TFALSE;
                if (bIsGood) {
